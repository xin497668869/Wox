...
namespace Wox.Plugin.Sys {
    public class Main : IPlugin, ISettingProvider, IPluginI18n {
        ...
        public List<Result> Query(Query query, Dictionary<string, int> historyHistorySources) {
            List<Result> commands = Commands();
            List<Result> results = new List<Result>();
            foreach (Result c in commands) {
                int titleScore = StringMatcher.Score(c.Title, query.Search);
                int subTitleScore = StringMatcher.Score(c.SubTitle, query.Search);
                int score = Math.Max(titleScore, subTitleScore);
                if (score > 0) {
                    c.Score = score;
                    results.Add(c);
                }
        ...
        private List<Result> Commands() {
            ...
            results.AddRange(new[] {
                new Result {
                    ...
                    Action = c => {
                        MessageBoxResult reuslt = MessageBox.Show("Are you sure you want to shut the computer down?",
                ...
                new Result {
                    ...
                    Action = c => {
                        MessageBoxResult result = MessageBox.Show("Are you sure you want to restart the computer?",
                ...
                new Result {
                    ...
                    Action = c => {
                        ...
                        // 0 for nothing
                        uint result = SHEmptyRecycleBin(new WindowInteropHelper(Application.Current.MainWindow).Handle,
                            0);
